...
# A importação de 'OrganizacaoBaixas' será feita localmente.

class GerarDocBaixa:
    def __init__(self, master, nome_planilha_base, dados_selecionados, dados_brutos_para_voltar):
        ...
        # Guarda a lista completa de dados para poder voltar para a tela anterior
    ...
    def exibir_tela(self):
        ...
        if self.brasao_para_gerarbaixa:
            lbl_brasao = ttk.Label(frame_cabecalho, image=self.brasao_para_gerarbaixa)
        ...
        ttk.Button(frame_rodape, text="<- Voltar e revisar agrupamentos", command=self.voltar_e_revisar, bootstyle="primary-outline").pack(side=LEFT)
        ttk.Button(frame_rodape, text="Confirmar e gerar Documentos", command=self.confirmar_geracao, bootstyle="success").pack(side=RIGHT)
    ...
    def voltar_e_revisar(self):
        """Recria a tela anterior para garantir a consistência dos dados e do foco."""
        from org_baixa import OrganizacaoBaixas
        
        # Destrói a janela atual para liberar o foco
        self.toplevel_gerarbaixa.destroy()
        
        # Acessa a janela que chamou a tela de organização (a janela de 'gerar_doc')
        janela_anterior = self.janela_mestra_gerarbaixa.master
        
        # Recria a tela de organização de baixas
        # ***** CORREÇÃO ABAIXO *****
        # Usando 'self.dados_brutos_originais' que é o nome correto do atributo.
        tela_anterior = OrganizacaoBaixas(janela_anterior, self.nome_planilha_base, self.dados_brutos_originais)
